...
For stronger machine discovery, also publish a site capability card at `/.well-known/agent-card.json`, add a `Link` header with `rel="service-desc"`, and keep `llms.txt` as supporting context rather than the only AI-facing surface.
...
- `/.well-known/agent-card.json` for the site capability card
- `Link: </.well-known/agent-card.json>; rel="service-desc"` on HTML responses
...
| Option | Type | Default | Description |
|---|---|---|---|
| `agentDiscovery` | `{ siteName?, description?, version?, siteUrl?, agentCardUrl?, llmsUrl?, preferExecution?, additionalSkills? }` | - | Optional overrides for the generated capability card and HTML discovery helpers |
