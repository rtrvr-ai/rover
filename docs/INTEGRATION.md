...
Guidance precedence is local-first: a visitor who hides narration or action highlights wins over all site and agent defaults; a visitor who shows them enables the feature locally; otherwise the site default comes from `experienceMode`, explicit shortcut/launch `runKind`, and owner controls. Action narration is generated by the backend Narration Director from the resolved action batch and Bridge target facts; legacy per-step `args.ui.narration` / `args.ui.highlight` values are stripped before execution and are not the presentation source. Narration and spotlight hints are presentation-only and do not enter planner/action prompts, schemas, or tool choice. Raw browser `tool_result` events are never spoken, but safe ACT/planner questions, checkpoints, and final responses may be narrated from the same user-facing response text shown in chat.
