...
1. Serve `agentCardJson` at `/.well-known/agent-card.json`.
2. Add `discoveryTags` near the Rover snippet in page source.
3. Add the `Link` header generated by `createRoverServiceDescLinkHeader(...)`.
...
| `agentDiscovery` | `{ siteName?, description?, version?, siteUrl?, agentCardUrl?, llmsUrl?, preferExecution?, additionalSkills? }` | — | Optional overrides for the generated capability card and discovery tags exposed to AI agents |
...
For stronger pre-task discovery, also publish a capability card at `/.well-known/agent-card.json`, add a `Link: </.well-known/agent-card.json>; rel="service-desc"` header, and include an inline capability-card tag generated by `createRoverAgentDiscoveryTags(...)`.
