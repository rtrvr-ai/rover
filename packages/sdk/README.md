...
| `ui.experience.audio.narration.enabled` | `boolean` | `true` | Owner gate for spoken narration through the SDK narrator. Browser speech is the default; entitled sessions may use the existing Rover narration audio stream. Visitors get a site-scoped speaker toggle when supported. |
| `ui.experience.audio.narration.defaultMode` | `'guided' \| 'always' \| 'off'` | `'guided'` | Speak guide flows by default, every run, or stay off unless the visitor enables it. A prompt submitted from microphone dictation opts into spoken output for that run only when owner narration is enabled and the visitor has not turned the speaker off. Response narration can also read safe ACT/planner questions, checkpoints, and final answers. |
...
Rover resolves a single `presentationIntent` (`'off' | 'task' | 'guide'`) per session, top wins: (1) visitor toggle, (2) shortcut `runKind`, (3) `?rover_guide=1` URL flag, (4) site default (`defaultMode: 'always'`), (5) voice-started guide, (6) prompt heuristics, (7) silent. For **guide runs**: planner and ACT each emit a one-sentence `<roverPresentation>{"speechText":"..."}</roverPresentation>` sidecar alongside their native function call. The server derives spotlight target (from the function call's `element_id`), groupKey dedup (from the tool kind), sensitivity (from the target label), captionTtlMs, and the narration provider — the model only owns the spoken sentence. Deterministic backend fallbacks fire only when the model emitted nothing valid. Planner narration explains the *purpose* of the next phase ("First I'll sign you in so we can see your dashboard."); ACT narration explains *what is happening on screen right now* ("Clicking Sign In."). For **task runs**, per-step narration is silenced — only the final `task_complete` answer, an `ask_user` question, and errors are spoken. For **off runs**, no narration prompt section is sent to the model. Legacy `args.ui.narration` / `args.ui.highlight` are stripped before execution. Narration and spotlight never enter tool argument schemas. Raw `tool_result` events stay silent.
...
| `deepLink.guideParam` | `string` | `'rover_guide'` | Boolean escalator — forces `runKind: 'guide'` for this run. Composes with `promptParam` / `shortcutParam`. Truthy: bare, `1`, `true`; falsy: `0`, `false`, `off`, `no`. |
...
- URL-fetch chatbot execution via `GET https://agent.rtrvr.ai/v1/a2w/runs`, `/v1/a2w/from-url`, and `/v1/a2w/go/<descriptor>`
...
For stronger pre-task discovery, publish `/.well-known/rover-site.json` as Rover's authoritative rich profile, publish `/.well-known/agent-card.json` as the interop card, add the generated `Link` header with `service-desc`, `service-doc`, `agent-run`, and `agent-resolver` relations, publish `/llms.txt`, and include inline discovery tags generated by `createRoverAgentDiscoveryTags(...)`. The generated card/profile/marker include `a2wGetEndpoint`, `fromUrlTemplate`, `deepLinkParams.executor = "rover_exec"`, per-shortcut `getRunUrl`, and `deepLinkWithExecutor`.
...
For chatbots that can only fetch URLs, use GET with cloud execution and markdown:

```text
https://agent.rtrvr.ai/v1/a2w/runs?url=https%3A%2F%2Fwww.rtrvr.ai&prompt=Get%20me%20the%20latest%20blog%20post&execution=cloud&wait=25&format=markdown
```
...
If the current page URL contains `rover_exec`, a chatbot should fetch that executor URL. If it contains `rover` or `rover_shortcut` without `rover_exec`, fetch:
...
Rover deep links like `?rover=` and `?rover_shortcut=` remain the simple browser-first entrypoints. Add `rover_exec` when a URL-only chatbot should execute in Rover cloud and receive markdown results. Cross-site workflows and handoffs extend that same public contract rather than replacing it.
